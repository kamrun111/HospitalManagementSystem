...
namespace HIMS.DAL
{
	public class CabinTranferDAL
	{
			...
			try
			{
				...
				while (dataReader.Read())
				{
					...
					if(dataReader["amount"] != DBNull.Value)
                        ocabin_tranfer.amount = Convert.ToDecimal(dataReader["amount"]);//service_amount

                    if (dataReader["service_amount"] != DBNull.Value)
                        ocabin_tranfer.service_amount = Convert.ToDecimal(dataReader["service_amount"]);
        ...
        public DataSet GetCabinTransferPopulationInfoICU(int patient_code, string cabin)
        {
            try
            {
                SqlParameter[] arParams = new SqlParameter[2];
                arParams[0] = new SqlParameter("@patient_code", patient_code);
                arParams[1] = new SqlParameter("@cabin", cabin);
                return DbHelper.GetDataSet(CommandType.StoredProcedure, "GetCabinTransferPopulationInfoICU", arParams);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public DataSet GetNonOccupiedCabinInfoICU()
        {
            try
            {
                return DbHelper.GetDataSet(CommandType.StoredProcedure, "GetNonOccupiedCabinInfoICU");
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public int Addcabin_tranferICU(CabinTranfer ocabin_tranfer)
        {
            SqlParameter[] arParams = new SqlParameter[3];
            try
            {
                ...
                return DbHelper.ExecuteNonQuery(CommandType.StoredProcedure, "cabin_tranferAdd_icu", arParams);
            }
            catch (Exception ex)
            {
                throw ex;
